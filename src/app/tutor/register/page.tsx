...
export default function TutorRegister() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect to tutor profile setup page
      router.push("/tutor/signup/subjects");
    } catch (error: any) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // If user already exists, they'll be redirected automatically
      // No need to check for needsRoleSelection since we're passing the role
      router.push("/tutor/signup/subjects");
    } catch (error: any) {
  ...
  return (
    <AuthLayout 
      ...
      subtitle="Start tutoring university students"
    >
      <div className="space-y-6">
        ...
        <div className="bg-green-50 p-4 rounded-md">
          <p className="text-sm text-green-800 mb-3">
            Are you a student looking for tutoring help?
          </p>
          <Link 
            href="/student/register"
            className="inline-block w-full text-center py-2 px-4 border border-green-500 rounded-md shadow-sm text-sm font-medium text-green-700 bg-white hover:bg-green-50 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
          >
